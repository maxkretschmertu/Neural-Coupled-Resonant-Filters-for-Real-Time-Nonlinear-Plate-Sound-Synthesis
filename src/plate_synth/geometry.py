...
def _shape_aspect(shape_mod: float, base_aspect: float = 1.0, max_stretch: float = DEFAULT_MAX_STRETCH) -> float:
...
def _triangle_vertices(shape_mod: float) -> np.ndarray:
    ...
    return np.array(
        [[0.0, 2.0 * height / 3.0], [-width / 2.0, -height / 3.0], [width / 2.0, -height / 3.0]],
        dtype=np.float64,
    )


def _polygon_radial_radius(theta: np.ndarray, vertices: np.ndarray) -> np.ndarray:
    ...
    for i in range(vertices.shape[0]):
        ...
        hit = valid_den & (t > 0.0) & (u >= -1e-12) & (u <= 1.0 + 1e-12)
...
def _periodic_interp(theta_query: np.ndarray, theta: np.ndarray, values: np.ndarray) -> np.ndarray:
    period = 2.0 * np.pi
...
def _unit_area_normalize(theta: np.ndarray, radius: np.ndarray) -> np.ndarray:
...
def _morphed_radius(theta: np.ndarray, morph: float, shape_mod: float) -> np.ndarray:
...
def _signed_distance_to_boundary(boundary_xy: np.ndarray, boundary_theta: np.ndarray, boundary_radius: np.ndarray, grid_size: int, domain_half_extent: float) -> tuple[np.ndarray, np.ndarray]:
    axis = np.linspace(-domain_half_extent, domain_half_extent, grid_size, dtype=np.float64)
    ...
    for start in range(0, points.shape[0], chunk):
        ...
        pa = p[:, None, :] - a[None, :, :]
        t = np.sum(pa * e[None, :, :], axis=2) / np.maximum(e2[None, :], 1e-20)
        t = np.clip(t, 0.0, 1.0)
    ...
    local_boundary = _periodic_interp(radial_angle, boundary_theta, boundary_radius)
    inside = point_radius <= local_boundary
    distance = np.sqrt(min_dist2)
    signed = np.where(inside, -distance, distance)
    return np.ascontiguousarray(signed.reshape(grid_size, grid_size)), np.ascontiguousarray(inside.reshape(grid_size, grid_size), dtype=bool)


def make_geometry(morph: float, shape_mod: float, grid_size: int = DEFAULT_SDF_GRID, boundary_samples: int = DEFAULT_BOUNDARY_SAMPLES, domain_half_extent: float = DEFAULT_DOMAIN_HALF_EXTENT) -> GeometryDescription:
    ...
    theta = np.linspace(0.0, 2.0 * np.pi, boundary_samples, endpoint=False, dtype=np.float64)
    radius = _morphed_radius(theta, morph, shape_mod)
    boundary = np.column_stack((radius * np.cos(theta), radius * np.sin(theta)))
    sdf, mask = _signed_distance_to_boundary(boundary, theta, radius, int(grid_size), float(domain_half_extent))
...
def boundary_radius_at(geometry: GeometryDescription, theta: np.ndarray | float) -> np.ndarray:
    return np.asarray(_periodic_interp(np.asarray(theta, dtype=np.float64), geometry.boundary_theta, geometry.boundary_radius), dtype=np.float64)
