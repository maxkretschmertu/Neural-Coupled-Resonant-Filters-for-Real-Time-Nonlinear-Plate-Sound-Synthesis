...
def make_material_grid(geometry: GeometryDescription, grid_size: int = 64) -> MaterialGrid:
    ...
    du = 2.0 / (grid_size - 1)
    weights = np.where(mask, rb * rb * du * du, 0.0)
    ...
    return MaterialGrid(
        ...
        integration_weights=np.ascontiguousarray(weights),
    )


def material_to_physical(geometry: GeometryDescription, u: np.ndarray | float, v: np.ndarray | float) -> tuple[np.ndarray, np.ndarray]:
...
def physical_to_material(geometry: GeometryDescription, x: np.ndarray | float, y: np.ndarray | float) -> tuple[np.ndarray, np.ndarray]:
...
def clamp_material_point(u: float, v: float, max_rho: float = 0.999) -> tuple[float, float]:
